...
function Drag() {
  ...
  function addDraggable() {
    if(name.trim()) {
        ...
        newState[1].push({id: `item-${name.trim()}-${new Date().getTime()}`, content: name.trim()})
        setState(newState);
    }
    setName('');
  }
  ...
  return (
    <div>
        <input placeholder="Insira nome" value={name} onChange={e => setName(e.target.value)}  />
        <button
            type="button"
                      ...
                      {(provided, snapshot) => (
                        ...
                        >
                          <div className="draggable-container">
                            ...
                            <button
                              type="button"
                              onClick={() => removeDraggable(ind, index)}
