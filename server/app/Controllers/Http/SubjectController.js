...
class SubjectController {
  ...
  async delete({request, response}){
    ...
    return response.status(200).send({ message: "Matéria apagada com sucesso" })
  }
  ...
  async update({request, response}){
    ...
    addedSetOfDisciplines.map(async ({module_id, total_classes})=>{
      ...
      const module = modules.filter(({module_id})=>{ return module_id === moduleID })
    ...
    modules.map(async ({module_id, total_classes})=>{
      const comparedModule = oldSetOfDisciplines.rows.find(({ $attributes }) => $attributes.module_id === module_id);

      const index = oldSetOfDisciplines.rows.indexOf(comparedModule);
      oldSetOfDisciplines.rows.splice(index,1);

      await SetOfDiscipline.query().where({subject_id, module_id}).update({total_classes});
    })

    oldSetOfDisciplines.rows.map(async ({$attributes})=>{
      await SetOfDiscipline.query().where({ subject_id: subject_id, module_id: $attributes.module_id }).delete();
    })

    await Subject.query().where('id', subject_id).update({name, abbreviation});

    return response.status(200).send({ message: "Matéria atualizada com sucesso" });
  }
}
