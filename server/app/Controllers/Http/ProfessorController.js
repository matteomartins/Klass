...
class ProfessorController {
  async store({ request, response }) {
    ...
    return response
      .status(201)
      .send({ message: "Professor criado com sucesso", id });
  }
  ...
  async show({ request }) {
    ...
    const {
      rows: [
        {
          $attributes: { subject_id: subjects },
        },
      ],
    } = await SubjectProfessor.query().where({ professor_id }).fetch();
    /*const subjects = allSubjects.map(({subject_id})=>{
      return subject_id
    });*/

    //selecionar os dias e horários
    const allDays = await Database.table("available_days").where({
      professor_id,
    });

    const days = await Promise.all(
      allDays.map(async ({ id, day }) => {
        const allSchedules = await Database.table("available_schedules").where(
          "available_days_id",
          id
        );
        const schedules = await Promise.all(
          allSchedules.map(({ start, end }) => {
            return { start, end };
          })
        );
  ...
  async index({ request }) {
    ...
    const professors = await Promise.all(
      listProfessors.rows.map(
        async ({ id, name, email, titulation, priority }) => {
          //selecionar as matérias
          const allSubjects = await Database.table("subject_professors").where({
            professor_id: id,
          });
          const subjects = allSubjects.map(({ subject_id }) => {
            return subject_id;
          ...
          const days = await Promise.all(
            allDays.map(async ({ id, day }) => {
              const allSchedules = await Database.table(
                "available_schedules"
              ).where("available_days_id", id);
              const schedules = await Promise.all(
                allSchedules.map(({ start, end }) => {
                  return { start, end };
                })
  ...
  async destroy({ request, response }) {
    const idProfessor = request.params.id;

    await Professor.query().where("id", idProfessor).delete();

    return response
      .status(200)
      .send({ message: "Professor apagado com sucesso" });
  }
  ...
  async update({ request, response }) {
    ...
    const oldSubjects = await SubjectProfessor.query()
      .where({ professor_id })
      .fetch();
    const subjectIDS = oldSubjects.rows.map(({ $attributes }) => {
      return $attributes.subject_id;
    });

    const addedSubjects = subjects.filter((value) => {
      if (!subjectIDS.includes(value)) return value;
    });
    ...
    subjectIDS.map(async (value) => {
      await SubjectProfessor.query()
        .where({ subject_id: value, professor_id })
        .delete();
    });
    ...
    return response
      .status(200)
      .send({ message: "Professor atualizado com sucesso" });
  }

  async userUpdate({ request, response }) {
    const professor_id = request.params.id_professor;
    const { titulation, days } = request.all();
    const allAvailableDays = await AvailableDay.query()
      .where({ professor_id })
      .fetch();
    const oldDays = allAvailableDays.rows.map(({ $attributes: { day } }) => {
      return day;
    });

    const addedDays = days.filter(({ day }, index) => {
      if (!oldDays.includes(day)) return days[index];
    });

    addedDays.map(async ({ day, schedules }) => {
      const thisDay = day;
      const removeDay = days.find(({ day }) => {
        return day === thisDay;
      });
      const index = days.indexOf(removeDay);
      days.splice(index, 1);
      const { id: available_days_id } = await AvailableDay.create({
        professor_id,
        day,
      });
    ...
    await Promise.all(
      days.map(async ({ day, schedules }) => {
        const [
          {
            $attributes: { id: available_days_id },
          },
        ] = allAvailableDays.rows.filter(
          ({ $attributes }) => $attributes.day === day
        );
        const allSchedules = await AvailableSchedule.query()
          .where({ available_days_id })
          .fetch();
        ...
        allSchedules.rows.map(async ({ $attributes }) => {
          await AvailableSchedule.query()
            .where({ id: $attributes.id })
            .delete();
        });

        const removeDay = allAvailableDays.rows.find(({ $attributes }) => {
          return $attributes.id === available_days_id;
        });
        const index = allAvailableDays.rows.indexOf(removeDay);
        allAvailableDays.rows.splice(index, 1);
        await AvailableDay.query()
          .where({ id: available_days_id })
          .update({ day });
      })
    );
