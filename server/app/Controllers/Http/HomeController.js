...
class HomeController {
    async index({ request, response, auth }) {
        ...
        const oldAllInnerJoin = await Database.table('schools').innerJoin('administrators', 'schools.id', 'administrators.school_id').innerJoin('courses', 'schools.id', 'courses.school_id').innerJoin('modules', 'courses.id', 'modules.course_id').innerJoin('groups', 'modules.id', 'groups.module_id').where('administrators.user_id', user_id)
